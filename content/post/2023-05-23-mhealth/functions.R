...
########################### plotting ###########################################
# main plotly fun
plotly_chloropleth <- function(data, z, tick_breaks = c(0.12, 0.14, 0.16, 0.18, 0.20), title = "", colors = "Oranges", symbol = "") {
  ...
  fig <- plot_geo(data, 
                  ...
                  hovertemplate = paste0(format(round(data[[z]] * 100, digits = 1), nsmall = 1), symbol, "<br>",
    ...
    add_trace(
      ...
      colorbar = list(ypad = 1, 
                      tickmode = "array",
                      ticktext = paste0(format(round(tick_breaks * 100, digits = 0), nsmall = 0), symbol),
